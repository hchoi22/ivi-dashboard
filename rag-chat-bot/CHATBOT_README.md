...
┌───────────────────────────────────────────────────────────────────────┐
│                     1. Source Documents                               │
│   • README files (artificially generated dataset summaries)           │
...
The documents ingested into this pipeline are limited to README files describing artificially generated dataset structure and published research papers for STUDY_A, STUDY_B, and STUDY_C. No subject-level data, raw clinical records, or real participant information is ingested into the vector database. This scope is intentional. If subject-level datasets are ever added to the ingestion path, this note and the privacy review behind it need to be revisited before that happens.
